...
function FolderOrder(props) {
	...
	const screen_name = "playlist_music";
	...
	const on_press = (e, pressed_column) => {
		try {
			...
			if (typeof playlist_music_order !== "undefined") {
				let list = JSON.parse(playlist_music_order).filter(
					item => item.id != id
				);
				list = [...list, { column: pressed_column, order: _order, id }];
				mmkv.set("PLAYER_PLAYLIST_MUSIC_ORDER", JSON.stringify(list));
			}
			callback();
		} catch (ex) {
			console.log(ex);
